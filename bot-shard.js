...
let i = client.shard.id
    ...
    for(i=0;i<client.shard.count;i++) {
        shardembed.addField(`<:civciv:836985951288557628>    **Shard ${i}**`, `<a:ayulemi:837023582906482699>    **Ping:** ${Math.round(shardinfo.ws.ping[i])}ms\n<a:yildiz:836614361304531024>   **Sunucu:** ${shardinfo.server_count[i]}\n <a:1097_sayorijump:833357907995131915>  **Kullanıcı:** ${shardinfo.user_count[i]}\n <:9852_Online_Status:837292523877171202>   **Uptime:** ${moment.duration(shardinfo.uptime[i]).format(`D [Gün] , H [Saat], m [Dakika], s [Saniye]`)} \n <:damam:837023540658044948>   **Bu Sunucunun Shardı Id'si**: ${client.shard.id}`, true)
    }
