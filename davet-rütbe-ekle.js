...
exports.run = async (client, message, args) => {
  ...
  if (veri2) {
    const embed = new Discord.MessageEmbed()
      .setDescription(`Zaten maksimum rütbe sınırına ulaşmışsınız! <a:ReddetmekGif:826398965268217876> `)
      .setColor("RED")
  ...
  if (!veri) {
    ...
    if (!sine) {
      const embed = new Discord.MessageEmbed()
        .setDescription(
          `Lütfen bir rol etiketleyiniz!\nÖrnek: ${prefix}rütbe-ekle @Qral 5 <a:ReddetmekGif:826398965268217876>`
        )
        .setColor("RED")
        .setFooter(client.user.username, client.user.avatarURL);
      message.channel.send(embed);
    ...
    const embed = new Discord.MessageEmbed()
      .setDescription(
        `Başarılı bir şekilde ${sine} rolü ${enis} davet karşılığında elde edilebilecek! <a:ModernOnayGif:827470162440617994>  `
      )
      .setColor("GREEN")
      .setFooter(client.user.username, client.user.avatarURL);
    message.channel.send(embed);
