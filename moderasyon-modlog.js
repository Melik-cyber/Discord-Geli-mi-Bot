...
let logkanal = await db.fetch(`log_${message.guild.id}`)
  ...
  if (args[0] === "sıfırla" || args[0] === "kapat") {
    if(!logkanal) return message.channel.send(`<a:ReddetmekGif:826398965268217876>  **Modlog Kanalı Zaten ayarlı değil**`);
    db.delete(`log_${message.guild.id}`)
   message.channel.send(` <a:ModernOnayGif:827470162440617994>  **ModLog Kanalı başarıyla sıfırlandı.**`);

    return
  }

if (!logk) return message.channel.send(` <a:ReddetmekGif:826398965268217876>  **Bir modlog kanalı belirtmelisin.**`);
